...
const MOCK_CLIENTS = [
  {
    ...
    aging: '31-60',
    status: 'Vencida',
    contact: 'Marta García',
    role: 'Directora Financiera',
    lastContact: '2023-10-25',
    email: 'marta.garcia@techsolutions.com',
    invoices: [
      { id: 'INV-2023-001', amount: 12000, due: '2023-09-15', status: 'Vencida', aging: '45 días' },
      { id: 'INV-2023-045', amount: 33200.50, due: '2023-11-15', status: 'Corriente', aging: '0 días' }
    ],
    history: [
      { id: 1, type: 'Llamada', date: '2023-10-25', note: 'Promesa de pago para finales de mes.', agent: 'Juan Pérez' },
      { id: 2, type: 'Email', date: '2023-10-20', note: 'Envío de recordatorio automático de primer aviso.', agent: 'Sistema' }
    ]
  },
  {
    id: '2',
    ...
    email: 'cruiz@distnorte.es',
    invoices: [
      { id: 'INV-2023-012', amount: 8500, due: '2023-06-10', status: 'Vencida', aging: '140 días' }
    ],
    history: [
      { id: 1, type: 'Reunión', date: '2023-10-28', note: 'Cliente reclama que la mercancía llegó dañada. Factura en disputa.', agent: 'Juan Pérez' }
    ]
  },
  {
    id: '3',
    name: 'Logística Global S.L.',
    nif: 'B-45678901',
    risk: 'Medio',
    riskScore: 54,
    totalDebt: 15400.00,
    ...
    contact: 'Ana Belén',
    role: 'Contabilidad',
    email: 'contabilidad@logglobal.com',
    lastContact: '2023-10-29',
    invoices: [
      { id: 'INV-2023-088', amount: 5400, due: '2023-10-10', status: 'Vencida', aging: '20 días' },
      { id: 'INV-2023-102', amount: 10000, due: '2023-12-01', status: 'Corriente', aging: '0 días' }
    ],
    history: []
  }
];
...
const RiskBadge = ({ risk }) => {
  ...
  return (
    <span className={`px-2 py-0.5 rounded-md text-[10px] font-bold border uppercase ${styles[risk]}`}>
      {risk}
    </span>
  );
};
...
const App = () => {
  ...
  return (
    <div className="flex h-screen bg-slate-50 text-slate-900 font-sans overflow-hidden">
      ...
      {/* Sidebar Compacto (Solo Logo e Info Usuario) */}
      <aside className="w-16 bg-slate-900 text-white flex flex-col items-center py-6 shrink-0">
        <div className="bg-indigo-600 p-2 rounded-xl mb-auto">
          <ShieldCheck className="w-6 h-6 text-white" />
        </div>
        <div className="w-10 h-10 rounded-xl bg-slate-800 flex items-center justify-center font-bold text-xs border border-slate-700">
          JP
        </div>
      </aside>

      {/* Área de Gestión de Cartera */}
        ...
        <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-8 shrink-0 z-10">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              <div className="p-2 text-slate-400 hover:text-indigo-600 cursor-pointer relative">
              ...
              {['Todos', 'Alto', 'Medio', 'Bajo'].map(r => (
                <button 
                  ...
                  className={`whitespace-nowrap px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-wider transition-all border ${
                    filterRisk === r ? 'bg-slate-800 text-white border-slate-800 shadow-md shadow-slate-200' : 'bg-white text-slate-500 border-slate-200 hover:bg-slate-100'
                  }`}
                >
                  {r}
                </button>
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
                      ...
                      <p className="text-2xl font-black text-slate-800">€{selectedClient.totalDebt.toLocaleString()}</p>
                    </div>
                    <div className="bg-white p-5 rounded-2xl border-l-4 border-l-rose-500 border border-slate-200 shadow-sm">
                      ...
                      <p className="text-2xl font-black text-rose-600">€{selectedClient.overdue.toLocaleString()}</p>
                    </div>
                    <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Rating de Cobro IA</p>
                      <div className="flex items-center gap-3 mt-1">
                        <div className="flex-1 h-2.5 bg-slate-100 rounded-full overflow-hidden">
                          <div 
                            className={`h-full transition-all duration-700 ${selectedClient.riskScore > 60 ? 'bg-emerald-500' : selectedClient.riskScore > 30 ? 'bg-amber-500' : 'bg-rose-500'}`}
                            style={{ width: `${selectedClient.riskScore}%` }}
                          ></div>
                        </div>
                        <span className="text-sm font-black">{selectedClient.riskScore}%</span>
                      </div>
                    </div>
                  </div>
                          ...
                          <tbody className="divide-y divide-slate-50 text-xs">
                            {selectedClient.invoices.map(inv => (
                              <tr key={inv.id} className="hover:bg-slate-50/50 transition-colors">
                                ...
                                <td className="px-6 py-5 text-right font-black text-slate-900">€{inv.amount.toLocaleString()}</td>
                                <td className="px-6 py-5 text-center">
                                  <span className={`px-2.5 py-1 rounded-lg text-[10px] font-black uppercase ${
                                    inv.status === 'Vencida' ? 'bg-rose-50 text-rose-600' : 'bg-slate-100 text-slate-500'
                                  }`}>
                                    {inv.status}
                                  </span>
                                </td>
                         ...
                         <div className="relative space-y-6 before:absolute before:left-[11px] before:top-2 before:bottom-2 before:w-[1.5px] before:bg-slate-100">
                            {selectedClient.history.length > 0 ? selectedClient.history.map((log) => (
                              <div key={log.id} className="relative pl-8">
                                <div className={`absolute left-0 top-1 w-[22px] h-[22px] rounded-full border-4 border-white shadow-sm flex items-center justify-center ${
                                  log.type === 'Llamada' ? 'bg-blue-500' : 'bg-slate-400'
                                }`}>
          ...
          <div className="relative bg-white w-full max-w-lg rounded-[32px] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 border border-white">
            ...
            <div className="p-8 space-y-6">
                ...
                <>
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-2">
                      ...
                      <input type="date" className="w-full bg-slate-50 border border-slate-200 p-4 rounded-2xl text-sm focus:ring-2 focus:ring-indigo-500 outline-none" />
                    </div>
                    <div className="space-y-2">
                      <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Importe Comprometido</label>
                      <input type="number" placeholder="0.00 €" className="w-full bg-slate-50 border border-slate-200 p-4 rounded-2xl text-sm focus:ring-2 focus:ring-indigo-500 outline-none font-bold" />
                    </div>
                  </div>
                  <div className="space-y-2">
              ...
              <div className="pt-4 flex gap-3">
                ...
                <button 
                  onClick={() => setIsActionModalOpen(false)}
                  className="flex-[2] py-4 text-xs font-bold text-white bg-indigo-600 hover:bg-indigo-700 rounded-2xl shadow-xl shadow-indigo-100 transition-all flex items-center justify-center gap-2"
                >
      ...
      {/* Acción Global (Solo si es necesario para el flujo) */}
      <button className="fixed bottom-8 right-8 w-14 h-14 bg-slate-900 text-white rounded-[20px] shadow-2xl flex items-center justify-center hover:scale-110 active:scale-95 transition-all group">
